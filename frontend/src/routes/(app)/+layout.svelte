...
{#if $authStore.user}
	<div class="h-screen max-h-screen w-full bg-slate-50/60 flex flex-col md:flex-row text-slate-800 relative overflow-hidden font-sans">
		...
		<main class="flex-grow h-full overflow-y-auto w-full relative z-10">
			<div class="px-4 sm:px-6 lg:px-8 xl:px-10 py-6 md:py-8 max-w-7xl mx-auto w-full flex flex-col transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]">
				<slot />
			</div>
		</main>
	</div>
