<script lang="ts">
	...
	async function refreshDashboard(showToast = false) {
		if (showToast && initialLoaded)
			toastStore.info("Refreshing dashboard...");
		await checkSystemState();
		await fetchDashboardData();
		await fetchStats();
		if (showToast && initialLoaded)
			toastStore.success("Dashboard metrics updated.");
	}
	...
	async function pauseBatch() {
		try {
			...
			toastStore.danger(
				err.response?.data?.message || "Failed to pause batch.",
			);
		}
	}
	...
	async function resumeBatch() {
		try {
			...
			toastStore.danger(
				err.response?.data?.message || "Failed to resume batch.",
			);
		}
	}
	...
	async function cancelBatch() {
		if (
			confirm(
				"Are you sure you want to cancel the active campaign dispatch? This cannot be undone.",
			)
		) {
			try {
				...
				toastStore.danger(
					err.response?.data?.message || "Failed to cancel campaign.",
				);
			}
		}
	}
	...
	{#if loading}
		...
		<div class="space-y-6">
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
				{#each Array(4) as _}
					<div
						class="p-1.5 bg-[#090d16]/5 border border-slate-200/40 rounded-2xl animate-pulse"
					>
						<div
							class="p-5 bg-white border border-slate-100 rounded-[calc(1rem-0.125rem)] shadow-soft h-[104px] flex items-center gap-4"
						>
							<div
								class="w-11 h-11 bg-slate-100 rounded-xl shrink-0"
							></div>
							<div class="space-y-2 flex-grow">
								...
								></div>
								<div
									class="h-6 bg-slate-100 rounded w-3/4"
								></div>
							</div>
						</div>
					</div>
			...
			<div
				class="p-1.5 bg-[#090d16]/5 border border-slate-200/40 rounded-[2rem] animate-pulse"
			>
				<div
					class="p-12 bg-white border border-slate-100 rounded-[calc(2rem-0.375rem)] shadow-soft flex flex-col items-center justify-center gap-4"
				>
					<div
						class="w-12 h-12 rounded-full border-4 border-slate-100 border-t-indigo-500 animate-spin"
					></div>
					<div class="space-y-2 text-center w-full max-w-sm">
						<div
							class="h-4 bg-slate-100 rounded w-2/3 mx-auto"
						></div>
						<div
							class="h-3.5 bg-slate-100 rounded w-1/2 mx-auto"
						></div>
					</div>
				</div>
			</div>
	...
	{:else}
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
			...
			>
				...
				>
					<div
						class="absolute -right-4 -bottom-4 w-16 h-16 rounded-full bg-emerald-50/20 opacity-40 group-hover:scale-125 transition-transform duration-500"
					></div>
					<div
						class="p-2.5 bg-emerald-50 border border-emerald-100 rounded-xl text-emerald-600 shrink-0 transition-transform duration-300 group-hover:scale-105"
					>
			...
			>
				...
				>
					<div
						class="absolute -right-4 -bottom-4 w-16 h-16 rounded-full bg-violet-50/20 opacity-40 group-hover:scale-125 transition-transform duration-500"
					></div>
					<div
						class="p-2.5 bg-violet-50 border border-violet-100 rounded-xl text-violet-600 shrink-0 transition-transform duration-300 group-hover:scale-105"
					>
						<Clock class="w-5 h-5" />
					</div>
					<div class="min-w-0">
			...
			>
				...
				>
					<div
						class="absolute -right-4 -bottom-4 w-16 h-16 rounded-full bg-indigo-50/20 opacity-40 group-hover:scale-125 transition-transform duration-500"
					></div>
					<div
						class="p-2.5 bg-indigo-50 border border-indigo-100 rounded-xl text-indigo-600 shrink-0 transition-transform duration-300 group-hover:scale-105"
					>
			...
			>
				...
				>
					<div
						class="absolute -right-4 -bottom-4 w-16 h-16 rounded-full bg-blue-50/20 opacity-40 group-hover:scale-125 transition-transform duration-500"
					></div>
					<div
						class="p-2.5 bg-blue-50 border border-blue-100 rounded-xl text-blue-600 shrink-0 transition-transform duration-300 group-hover:scale-105"
					>
				...
				>
					...
					>
						<div>
							...
							<h3
								class="font-extrabold text-lg text-slate-900 tracking-tight"
							>
					...
					<div class="space-y-3 pt-2">
						<div class="flex justify-between text-xs font-semibold">
							...
							>
							<span
								class="text-indigo-600 font-bold tracking-tight"
								>{percentComplete}% Complete</span
							>
						</div>
						<div
							class="w-full bg-slate-100 rounded-full h-4 overflow-hidden p-0.5 border border-slate-200/50 shadow-inner"
						>
							<div
					...
					<!-- Secondary details -->
					<div
						class="grid grid-cols-2 sm:grid-cols-4 gap-6 pt-6 border-t border-slate-100 text-xs"
					>
						<div class="space-y-1">
							<span
								class="text-slate-400 block font-bold uppercase tracking-wider text-[9px]"
								>Delivered</span
							>
							<span
		...
		{#if scheduledJobs.length > 0}
			...
			>
				...
				>
					...
					>
						<h3
							class="font-extrabold text-slate-900 text-base tracking-tight"
						>
							Scheduled Campaigns
						</h3>
						<span
							class="bg-slate-100 text-slate-600 text-[10px] font-bold px-2.5 py-1 rounded-lg"
						>
					...
					<div class="overflow-x-auto -mx-6 md:mx-0">
						<table
							class="w-full text-left border-collapse min-w-[650px] md:min-w-0"
						>
							<thead>
								<tr
									class="text-[10px] uppercase text-slate-400 font-bold border-b border-slate-100/80 bg-slate-50/50"
								>
									<th
									...
									>
									<th
										class="px-6 py-4 font-bold tracking-wider text-right"
										>Actions</th
									>
								</tr>
							</thead>
							<tbody class="divide-y divide-slate-100/60 text-xs">
								{#each scheduledJobs as job (job.id)}
									<tr
										class="hover:bg-slate-50/40 transition-all duration-200 group"
									>
										<td
											class="px-6 py-4 font-bold text-slate-800"
											>{job.subject || "Campaign"}</td
										>
										<td
										...
										>
										<td
											class="px-6 py-4 text-slate-700 font-semibold"
											>{job.contact_count ||
												"All contacts"}</td
										>
										<td
											class="px-6 py-4 text-slate-500 font-medium"
											>{job.notify_email || "None"}</td
										>
										<td class="px-6 py-4">
											{#if job.status === "running"}
												<span
													class="inline-flex items-center gap-1.5 bg-blue-50 border border-blue-100 text-blue-600 font-extrabold text-[9px] tracking-wider uppercase px-2.5 py-1 rounded-full animate-pulse"
												>
													<span
											...
											{:else}
												<span
													class="inline-flex items-center gap-1.5 bg-amber-50 border border-amber-100 text-amber-600 font-extrabold text-[9px] tracking-wider uppercase px-2.5 py-1 rounded-full"
												>
													<span
														class="w-1.5 h-1.5 bg-amber-400 rounded-full"
													></span>
													Scheduled
												</span>
											{/if}
		...
		{#if !batchStatus && scheduledJobs.length === 0}
			...
			>
				...
				>
					...
					<div
						class="relative w-24 h-24 mb-6 flex items-center justify-center bg-slate-50 border border-slate-100 rounded-3xl shadow-sm"
					>
						<!-- Mail icon vector backdrop -->
						<div
							class="absolute inset-0 bg-gradient-to-tr from-primary/5 to-secondary/5 rounded-3xl opacity-60"
						></div>
						<div
							class="absolute -top-2 -right-2 p-1.5 bg-emerald-500 text-white rounded-xl shadow-sm shadow-emerald-500/10"
						>
					...
					>
						All Campaigns Dispatched
					</h3>
					<p
						class="text-slate-500 text-xs md:text-sm mt-2 max-w-sm leading-relaxed font-medium"
					>
						There are no campaigns currently active or scheduled in
						the queue. You can compose a new one to start sending.
					</p>
					...
					>
						...
						<span
							class="w-5 h-5 rounded-full bg-white/20 flex items-center justify-center transition-transform duration-300 group-hover:translate-x-1"
						>
							<svg
								class="w-3 h-3 text-white fill-none stroke-current"
								stroke-width="2.5"
								viewBox="0 0 24 24"
